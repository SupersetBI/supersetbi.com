...
安装之后，您应该能够将浏览器指向正确的 主机名:端口 `http://localhost:8088 <http://localhost:8088>`_,
...
除了 Sqlite ( Python 标准库的一部分)之外，Superset 没有绑定到数据库的连接。
您将需要安装作为元数据数据库使用的数据库所需的包，以及通过 Superset 连接到要访问的数据库所需的包。
...
注意，还支持许多其他数据库，主要的标准是是否存在 SqlAlchemy dialect 和 Python 驱动程序。
...
至少需要 escape/encode s3_staging_dir, 即, ::
